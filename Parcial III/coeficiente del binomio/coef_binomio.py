...
coeficiente del binomio, según la formula mostrada anteriormente.

Ejemplo de los archivos de entrada y salida.
Datos.txt Resultados.txt
4, 7
2, 3
5, 9
...
def coeficientedelbinomio(n, k):
    coef_binomio = factorial(n) / (factorial(k) * factorial(n - k))
    return coef_binomio
...
def imprimir(arch, n, k, c):
    arch.write ("coeficiente " + str(k) + " para binomio de " + str(n) + " es: " + str(c) + "\n")
